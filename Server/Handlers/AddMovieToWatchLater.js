...
const addMovieToWatchLater = async (request, response) => {
  //_id is the movie id
  //qty is the amount of items chosen by the user in the FE
    const { _id, title, vote_average, overview, poster_path } = request.body;
  ...
  //get the user Id from the local storage on the FE
    ...
    if (!_id || !title || !vote_average || !overview || !poster_path) {
        return response
        .status(400)
        .json({ status: 400, message: "Missing movie data" });
    }
    ...
    if (!containsItem) {
        const movieData = { _id, title, vote_average, overview, poster_path };

        itemsArray.push(movieData);
...
module.exports = { addMovieToWatchLater };
